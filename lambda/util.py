...
def add_question(question, passage = ''):
    passage.append('{}')
    return passage.append(question)
    #return '</lang></voice> {} </speak>'.format(data.QUESTION_LIST[question_number])

def speak_in_english(phrase):
    return '<speak><voice name="Amy"><lang xml:lang="en-GB">'+str(phrase)+'</lang></voice></speak>'

def in_english(phrase):
    return '<voice name="Amy"><lang xml:lang="en-GB">'+str(phrase)+'</lang></voice>'

def choose_passage(passage_number):
    return data.LISTENING_PASSAGE[passage_number]
